...
def Generate_Spatial_Operators(x_mesh,scheme,derivation_order):
    ...
    if scheme == "2nd-order-central":
        for i, x_eva in enumerate(x_mesh):
            if i == 0:
                x_sten = x_mesh[:3]
                D[i, :3] = Generate_Weights(x_sten, x_eva, derivation_order)

            elif i == N - 1:
                x_sten = x_mesh[-3:]
                D[i, -3:] = Generate_Weights(x_sten, x_eva, derivation_order)
            else:
                x_sten = x_mesh[i - 1:i + 2]
                D[i, i - 1:i + 2] = Generate_Weights(x_sten, x_eva, derivation_order)
    ...
    if scheme == "2nd-order-upwind": # also called QUICK
        ...
        # A_circulant = scylinalg.circulant(circulating_row)
        # D = A_circulant.transpose()
        # change the first and second row of A here
        ...
        for i, x_eva in enumerate(x_mesh):
            if i == 0:
                ...
                x_sten = x_mesh[i - 2:i + 1]
                D[i, i - 2:i + 1] = Generate_Weights(x_sten, x_eva, derivation_order)
