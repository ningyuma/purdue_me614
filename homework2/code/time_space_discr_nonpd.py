...
def p3_a_ply_get_delta(Nx, Lx, c_x, alpha, beta, w, a, num_curve, order, get_del=False):
    ...
    # A, B = scysparse.csr_matrix(A), scysparse.csr_matrix(B)
        if it % plot_every == 0:  # plot every plot_every time steps  np.mod(x1,x2)=x1-floor(x1/x2)*x2
            plt.figure(order)
            if thr <7:
                plt.plot(x_mesh[1:-1], u[1:-1], label="Time = " + str(it * dt) + "s")
                delta[thr] = get_delta(u, a, x_mesh, Lx)

            elif thr >= 7 and thr < 14:
                plt.plot(x_mesh[1:-1], u[1:-1], '--', label="Time = " + str(it * dt) + "s")
                delta[thr] = get_delta(u, a, x_mesh, Lx)
            elif thr == num_curve -1:
                plt.plot(x_mesh[1:-1], u[1:-1], '-.', label="Final Time")
                delta[thr] = get_delta(u, a, x_mesh, Lx)
            else:
                plt.plot(x_mesh[1:-1], u[1:-1], '-.', label="Time = " + str(it * dt) + "s")
                delta[thr] = get_delta(u, a, x_mesh, Lx)
